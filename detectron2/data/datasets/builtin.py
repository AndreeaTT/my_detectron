...
from .malaga import register_malaga
...
# ==== Predefined splits for PASCAL VOC ===========
def register_all_malaga(root="datasets"):
    SPLITS = [
        ("Malaga_train", "Malaga", "train"),
        ("Malaga_val", "Malaga", "val"),
        ("Malaga_test", "Malaga", "test"),
    ]

    for name, dirname, split in SPLITS:
        register_malaga(name, os.path.join(root, dirname), split, year=2007)
        MetadataCatalog.get(name).evaluator_type = "pascal_voc"
